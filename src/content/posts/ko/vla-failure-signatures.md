...
이 글은 **[1]에서 [3]까지**, 관측에서 감시 지표를 고르는 데까지를 다룹니다. 무엇을 감시할지가 정해져야 [4]에서 무엇을 막을지도 정해지기 때문입니다. [4]는 마지막에 **문만 열어둡니다** — 막는 일이 공짜가 아니라는 것까지만 짚고 넘어갑니다.

재료는 arXiv 프리프린트 [How VLAs Fail Differently](https://arxiv.org/abs/2605.28726) 한 편이고, 그중 발견 하나만 따라갑니다 — **같은 과제에서 성적이 비슷한 두 로봇 정책이, 실패할 때는 정반대 모양으로 무너졌다**는 것입니다.

미리 밝혀둘 것이 셋 있습니다. 이 논문은 **동료 심사를 거치지 않은 프리프린트**이고, **소속 없는 연구자 한 명이 단독으로 낸 글**이며, 실험은 **전부 시뮬레이션**입니다. 실물 로봇 검증은 저자도 향후 과제로 남겨두었습니다. 그래서 이 글은 결론을 빌려오는 대신, 저자가 무엇을 보고 그렇게 말했는지를 따라갑니다.
...
연속 모델은 반대입니다. 노이즈를 깎아 그럴듯한 궤적의 한가운데로 수렴하는데, 관측이 애매해서 **그럴듯한 궤적들이 서로 반대쪽을 가리키면 그 한가운데는 "거의 움직이지 않음"이 됩니다.** 왼쪽으로 갈 이유와 오른쪽으로 갈 이유가 비슷하면 평균이 0 근처에 놓입니다. 고장 나서 멈추는 게 아니라, **정지가 가장 그럴듯한 답이 되어버리는 것**입니다.
...
**속도 한계 위반은 어디서도 통하지 않습니다.** 그리고 이게 배포 코드에서 가장 흔히 쓰이는 안전장치입니다. ACT가 결정적입니다 — 위반이 **4,758건**이나 나는데 AUROC는 0.52, 사실상 동전 던지기입니다. 논문의 설명은 이렇습니다. ACT의 실패는 **위험한 동작 때문이 아니라 물리적으로 멀쩡해 보이는 틀린 동작 때문**이고, 속도로는 잘 하는 쪽과 못 하는 쪽이 구분되지 않습니다.

**그리고 여기가 이 글에서 가장 뜻밖이었던 곳입니다 — 정지율이 0.50, 0.49, 0.53입니다.** 셋 다 동전 던지기입니다.

이 글은 117 대 1이라는 정지 스텝 차이로 시작했습니다. 두 모델을 가장 선명하게 갈라놓은 숫자였습니다. 그런데 **그 선명한 증상을 감시자로 쓰면 아무것도 못 잡습니다.** 정지는 아키텍처를 구별해주지만 실패를 구별해주지는 않습니다. Diffusion은 성공할 때도 잠깐씩 멈추기 때문입니다.

눈에 잘 띄는 증상과 쓸 만한 지표가 다르다는 것 — 이게 이 표가 알려주는 것 중 실무에 가장 오래 남을 내용이라고 생각합니다.
...
숫자를 읽을 때 한 가지는 감안해야 합니다. 논문이 붙인 95% 신뢰구간을 보면 ACT의 방향 반전율은 **0.91 [0.79, 0.99]** 로 폭이 넓습니다. 50 에피소드밖에 안 돌렸기 때문입니다. 반면 VQ-BeT는 **0.93 [0.88, 0.97]** 로 좁습니다. 셋 다에서 통했다는 결론은 유지되지만, ACT 쪽은 아직 표본이 얇습니다.

---

## ④ 감시자를 붙이면 공짜인가

논문은 여기서 한 걸음 더 갑니다. 탐지한 다음 실제로 명령에 개입하는 계층(논문에서는 SafeContract라고 부릅니다)을 붙여보고, 성공률이 어떻게 되는지를 쟀습니다.
...
이것이 [4] 단계의 실제 모양입니다. 막는 일은 공짜가 아니고, **얼마나 잘 잡느냐가 아니라 얼마를 내주고 잡느냐**의 문제가 됩니다. 이 글에서는 여기까지만 짚고 넘어갑니다.
...
전통적인 안전 리미터는 한계선을 넘는지를 봅니다. 그런데 AI의 실패는 **한계 안에서 조용히** 일어납니다. ACT에서 4,758번 울린 경보가 성공과 실패를 거의 갈라내지 못했다는 사실이 그것을 보여줍니다. 경보가 틀렸다기보다, 그 경보가 재고 있던 것이 애초에 실패와 상관이 적었습니다.

그리고 눈에 가장 잘 띄던 증상 — 117 대 1의 정지 — 은 감시자로는 쓸모가 없었습니다. **선명한 것과 쓸 만한 것은 다릅니다.**

한 가지 덧붙이면, 이 글이 따라간 논문은 심사를 거치지 않은 단독 저자의 프리프린트이고 실험도 전부 시뮬레이션입니다. 그러니 여기 숫자를 결론으로 들고 가기보다, **어떤 질문을 해야 하는지의 목록으로** 쓰는 편이 맞다고 생각합니다. 저는 그렇게 쓸 생각입니다.
